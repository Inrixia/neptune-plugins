...
export const DropdownSelect = ({ selected, onSelect, options, title, tooltip }: DropdownSelectProps) => {
	...
	return html`
		<${DivWithTooltip} tooltip=${tooltip}>
			...
			<select id="dropdown-${title}" value=${selected} onChange=${onChange} style="flex-grow: 1; max-width: 180px; background: var(--wave-color-solid-base-brighter);border-bottom: 1px solid var(--wave-color-opacity-contrast-fill-ultra-thin); color: var(--wave-color-opacity-contrast-fill-t);">
